...
async function updateMyRoutes() {
  ...
  // Remove and redraw (simple and robust).
  // Why: route can appear later; redrawing avoids "stale partial" state.
  myRoutePre = removeIfExists(myRoutePre);
  myRouteRide = removeIfExists(myRouteRide);
  myRoutePost = removeIfExists(myRoutePost);
  myWalkToPickup = removeIfExists(myWalkToPickup);
  ...
  myWalkToPickup = L.polyline(w1, { weight: 4, opacity: 0.85, dashArray: "6" }).addTo(map).bindTooltip("Walk to pickup");
  myWalkFromDropoff = L.polyline(w2, { weight: 4, opacity: 0.85, dashArray: "6" }).addTo(map).bindTooltip("Walk from dropoff");

  myPickup = L.circleMarker(pickup, { radius: 7, weight: 2, fillOpacity: 1 }).addTo(map).bindTooltip("Pickup");
  myDropoff = L.circleMarker(dropoff, { radius: 7, weight: 2, fillOpacity: 1 }).addTo(map).bindTooltip("Dropoff");

  // Fit view once routes exist
  const all = d.concat(w1, w2);
  if (all.length > 0) map.fitBounds(all, { padding: [30, 30] });
}
...
function stopViewLoops() {
  ...
  if (routesTimer) { clearInterval(routesTimer); routesTimer = null; }
}
